	...
	while 1: # Loop forever so the generator never terminates
		...
		for offset in range(0, num_samples, batch_size):
			...
			for batch_sample in batch_samples:
				...
				# lImage_aug = cv2.flip(lImage, 1)
				# images.append(lImage_aug)
				# measurements.append(-lMeasurement)

				# rImage_aug = cv2.flip(rImage, 1)
				# images.append(rImage_aug)
				# measurements.append(-rMeasurement)
...
# # Preprocess incoming data, centered around zero with std
# model.add(Lambda(lambda x: x / 127.5 - 1., input_shape=(160, 320, 3)))
# model.add(Cropping2D(cropping=((52, 20), (0, 0)), input_shape=(160, 320, 3)))
